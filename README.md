...
```bash
docker run -itd --name gui-cnt_1.0 --env="NVIDIA_DRIVER_CAPABILITIES=all" --gpus '"device=1"' --shm-size=2g -p 2222:22 -p 3389:3389 -p 1234:1234 -p 1235:1235 gui-img:1.0 zsh
```
...
Don't forget preliminary to specify `export DISPLAY=:0.0` (or whatever)
```bash
vulkaninfo | grep -E "apiVersion|deviceName|driverID"
```
In XFCE terminal 
```bash
# export __GLX_VENDOR_LIBRARY_NAME=nvidia # Uncomment and put it to ~/.myrc if you use NVIDIA
...
* You can specify additional `docker run` params and expand ports mapping
    ```bash
    -ipc=host \
    ...
    ```bash
    -p 2318:22/tcp \
    -p 2319:3389/udp -p 2319:3389/tcp \
...
* If needed [install *cuda-toolkit*](https://developer.nvidia.com/cuda-downloads?target_os=Linux&target_arch=x86_64&Distribution=Ubuntu&target_version=24.04&target_type=deb_network) with specific version inside container and put:
    ...
    echo 'export PATH=$CUDA_HOME/bin:$PATH' >> ~/.myrc
    echo 'export LD_LIBRARY_PATH=$CUDA_HOME/lib64:$LD_LIBRARY_PATH' >> ~/.myrc
    ```